...
- Multi-contributor Repos: ${
      repoDetails.filter((r) => r.collaboration_metrics.total_contributors > 1)
        .length
    }
- Total Commits: ${repoDetails.reduce(
...
- Repos with Tests: ${
      repoDetails.filter((r) => r.code_quality.has_tests).length
    }
- Repos with CI: ${repoDetails.filter((r) => r.code_quality.has_ci).length}
...
- ${repo.name}:
  ...
  * README Sections: ${
    repo.readme_metrics?.sections?.map((s: any) => s.name).join(", ") || "None"
  }
  ...
  * README Content:
  """${repo.readme_metrics?.content || "Not available"}"""
`
  )
  .join("")}
